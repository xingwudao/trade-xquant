...
运行状态、callback、连接故障和常见排查见
`docs/05-qmt-runtime-notes.md`。

## 6. 任务生命周期

任务生命周期从 Xquant pending task 开始：

1. daemon 或 `poll-once` 拉取 pending task。
2. 本地 SQLite 记录 task claim，保证幂等。
3. 查询 QMT 账户、持仓和价格。
4. `PortfolioEngine` 生成订单计划。
5. `RiskControl` 校验账户、过期时间、权重、价格、金额、换手率和交易时段。
6. dry-run 只记录计划和结果。
7. real mode 在双安全门打开后下单。
8. 本地记录计划、委托、事件、成交和结果。
9. Xquant 接收计划和结果。
...
- active condition order polling。
- submitted / partial order sync。
- heartbeat。
- QMT 账户状态同步。

即使 Xquant 没有新交易任务，daemon 仍需要同步 condition order、
已提交订单、成交结果和 heartbeat。订单生命周期详见
...
- 全部成交: `success`。
- 部分成交或仍有 pending: `partial`。
- 未成交且未超时: `submitted`。
- 失败或拒单: 记录失败信息。

超时后只把 QMT 当前仍 pending 的订单作为撤单候选。撤单成功后，
daemon 刷新账户、持仓和价格，并在 retry budget 内按原目标权重重试。

安全要求：

- 历史 `submitted_orders` 不能授权再次撤单。
- 撤单失败不重试，且 live order 继续可同步。
- preflight 阻止不消耗 retry budget。
- accepted live order 不能被 terminal failed 隐藏。
- terminal failure 和 terminal no-op 上报失败必须可补报。

详见 `docs/06-daemon-order-lifecycle.md`。

## 10. Condition Order 设计

condition order 用于止损、止盈和 trailing 类规则。
...
- 组合级止损。
- 组合级止盈。
- 静态百分比、固定比例 trailing、ATR、HV、标准差等参数化规则。

condition task 也可能产生真实委托。其 submitted / partial 生命周期遵守
普通任务同样的安全规则，但 retry result 必须走 condition result path。

规则体系见 `docs/07-conditional-stop-take-profit-rules.md`。

## 11. 本地 Condition Order JSON

本地 condition order JSON 用于开发和联调，不替代正式 Xquant task contract。

它支持：

- 从本地文件加载 condition order。
- 本地模拟触发。
- 使用 mock broker 或 QMT broker 走同样执行链路。

JSON 结构和示例见 `docs/08-local-condition-orders.md`。
...
任务结果 payload 中会保留 account snapshot、submitted orders、trades、
events、errors 和 lifecycle metadata。常用查询见
...
- daemon polling 和 heartbeat。
- submitted / partial sync。
- 超时撤单和 retry。
- retry budget。
- cancel failure。
- terminal result 补报。
- condition task result path。
...
后续新增专题文档必须继续纳入该体系，例如：

- `docs/11-storage-audit-design.md`
- `docs/12-testing-and-verification.md`
