...
有效交易 session 的判断来自 Xquant `CN_A` 交易日历缓存，而不是本地
按星期和时间猜测。daemon 会批量拉取本年度剩余日历到 SQLite，实盘 gate
只读本地缓存：当天必须是交易日，且当前时间位于当天 sessions 内。缓存
缺失、刷新失败或服务端缺少已发布日历时，按不可交易处理。
